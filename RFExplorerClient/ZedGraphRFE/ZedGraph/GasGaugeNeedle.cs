...
namespace ZedGraph
{
	...
	/// </summary>
	[Serializable]
    public class GasGaugeNeedle : CurveItem, ICloneable, ISerializable, IDisposable
	{
	#region Fields
		...
		/// <summary>
		/// The style of the tip of the needle
		/// </summary>
        ...
        /// <summary>
        /// true if we want the needle to get the color from the region
        /// </summary>
        private bool _dynamicColor;
	...
	#region Constructors
		...
		{
			NeedleValue = val;
			NeedleColor = Default.NeedleColor;
			NeedleWidth = Default.NeedleWidth;
            _dynamicColor = Default.DynamicColor;
            _color = color;
			LineCap = Default.LineCap;
			SweepAngle = 0f;
	...
	#region Properties
		...
		/// <summary>
		/// Gets or Sets the LineCap of this <see cref="GasGaugeNeedle"/>
		/// </summary>
	...
	#region Default
		...
		public struct Default
		{
			...
			/// <summary>
			/// The default color of the gas gauge needle.
			/// </summary>
			public static Color NeedleColor = Color.Black;
			
			/// <summary>
			/// The default cap of the gas gauge needle
			/// </summary>
			public static LineCap LineCap = LineCap.Round;
            ...
            /// <summary>
            /// Default dyn setting for colors
            /// </summary>
            public static bool DynamicColor = false;
			...
			foreach ( CurveItem curve in pane.CurveList )
			{
				if ( curve is GasGaugeNeedle )
				{
					...
					float sweep = ( (float)ggn.NeedleValue - (float)minVal ) /
						( (float)maxVal - (float)minVal ) * (pane.Angle);
					if(pane.Clockwise)
						ggn.SweepAngle = pane.Angle-sweep - (pane.Angle-180)/2;
					else
						ggn.SweepAngle = sweep - (pane.Angle-180)/2;
				}
			}
		}
