...
namespace ZedGraph
{
    ...
    public class ArrowObj : LineObj, ICloneable, ISerializable
    {
        ...
        bool m_bEmptyArrowOnly = false;
        public bool EmptyArrowOnly
        {
            get { return m_bEmptyArrowOnly; }
            set { m_bEmptyArrowOnly = value; }
        }
        ...
        override public void Draw( Graphics g, PaneBase pane, float scaleFactor )
        {
                ...
                pix2.X > -10000 && pix2.X < 100000 && pix2.Y > -100000 && pix2.Y < 100000 )
            {
                ...
                if (!EmptyArrowOnly)
                    scaledSize = (float)( _size * scaleFactor );
                    ...
                    //new Pen( _color, pane.ScaledPenWidth( _penWidth, scaleFactor ) ) )
                {
                    ...
                    if ( _isArrowHead )
                    {
                        ...
                        if (EmptyArrowOnly)
                        {
                            //render a single, empty arrowhead
                            ...
                            if (!String.IsNullOrEmpty(m_sInsideText))
                            {
                                using (SolidBrush brush = new SolidBrush(_line._color))
                                {
                                    using (Font font = new Font("Arial", 8))
                                    {
                                        ...
                                        g.DrawString(m_sInsideText, font, brush, new RectangleF(-hsize, -scaledSize, 2 * hsize, scaledSize), drawFormat);
